...
import { normalizeImageExt } from "@/lib/image-keys";
import {
	deleteImageBySlug,
	insertImage,
	listAssignableTags,
...
import { parseEmptyInput, parseUploadInput, type UploadSource } from "@/server/upload-image-input";
...
export const listAssignableTagsFn = createServerFn({ method: "GET" })
	.inputValidator((input: unknown) => parseEmptyInput(input))
	.handler(async () => {
